...
myd <- myd |> 
  ...
  ungroup()
```
このスケーリングやめる？線形性があるので，特に意味はないはず．<-資産が0なのは未発見．

ACCが0というのは，100万未満のデータが売り切られているので，1を代入するのは？<- quantile regで対処はできているはず．
...
```{r}
#分位点回帰をしてみる
rq(y_q ~ x_1_q + x_2_q, data = myd)
```

残差の計算
```{r}
myd <- myd |> 
  mutate(u_q_unsc = y_q - (-1.358538e+02 -7.427515e-03*x_1_q - 7.108090e-02*x_2_q))
```
Infはなくなったね！
